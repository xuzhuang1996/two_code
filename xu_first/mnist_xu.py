...
# Construct model
pred = conv_net(x, weights, biases, keep_prob)#Tensor("Add_1:0", shape=(?, 10), dtype=float32)
...
with g.as_default():
    ...
    #tf.train.write_graph(graph_def, EXPORT_DIR, 'mnist_model1.pb', as_text=False)

    output_graph_def = tf.graph_util.convert_variables_to_constants(sess, sess.graph_def, output_node_names=['output'])
    with tf.gfile.FastGFile('mnistx.pb', mode='wb') as f:
